...
################Save PDB database as PDB or database!#########################################################################  
    ...
    def save_cur_as_pdb(self, outpath, supress_modelSep=False):
        ...
        FILE = open_file(outpath, 'w')
        result = self._convert_and_save_to_FILE_pdbformat(self.cur, FILE, supress_modelSep)
        FILE.close()
        if not result:
            print outpath+" is missing it's renumbered PDB or there was a problem copying it to the sqlite3 coordinate table"
            os.remove(outpath)
            return False
        return True
